...
# Count domains with each tag and calculate percentages
echo "TAG USAGE:"
...
if [ $total_domains -gt 0 ]; then
    ...
    echo "  Domains with 'issue':         $issue_count ($issue_percent%)"
    echo "  Domains with 'issuewild':     $issuewild_count ($issuewild_percent%)"
    echo "  Domains with 'iodef':         $iodef_count ($iodef_percent%)"
fi
...
if [ $total_domains -gt 0 ]; then
    ...
    echo "  Domains with 'issue' AND 'issuewild': $both_issue_issuewild ($both_percent%)"
    echo "  Domains with all three tags (issue, issuewild, iodef): $all_three_tags ($all_three_percent%)"
fi
...
# Most used CAs (with merged entries)
echo ""
echo "MOST USED CERTIFICATE AUTHORITIES:"
jq -r '[.[] | 
  .results.CAA.data.answers[]? | 
  select(.tag == "issue" or .tag == "issuewild") | 
  select(.value != null and .value != "" and .value != ";") | 
  .value | 
  tostring | 
  # Extract just the CA domain, removing parameters
  split(";")[0] | 
  gsub("^[[:space:]]+|[[:space:]]+$";"") |
  select(. != "")
] | group_by(.) | 
  map({ca: .[0], count: length}) | 
  sort_by(.count) | 
  reverse | 
  .[] | 
  "  \(.ca): \(.count)"' "$TEMP_FILE" | head -15
...
if [ $total_domains -gt 0 ]; then
    multi_ca_percent=$(echo "scale=2; $multi_ca_count * 100 / $total_domains" | bc)
    echo "DOMAINS WITH MULTIPLE CAs: $multi_ca_count ($multi_ca_percent%)"
else
...
jq -r '.[] | {
  ...
  ca_list: ([.results.CAA.data.answers[]? | select(.tag == "issue" or .tag == "issuewild") | select(.value != null and .value != "" and .value != ";") | .value | tostring | split(";")[0] | gsub("^[[:space:]]+|[[:space:]]+$";"")] | unique | join(";"))
} | [.domain, .has_caa, .issue_count, .issuewild_count, .iodef_count, .ca_list] | @csv' "$TEMP_FILE" >> "$CSV_FILE"

# Generate separate file for domains with multiple CAs (with merged CA names)
MULTI_CA_FILE="${OUTPUT_PREFIX}_multiple_cas.txt"
jq -r '.[] | {
  domain: .name,
  cas: [.results.CAA.data.answers[]? | select(.tag == "issue" or .tag == "issuewild") | select(.value != null and .value != "" and .value != ";") | .value | tostring | split(";")[0] | gsub("^[[:space:]]+|[[:space:]]+$";"")] | unique
